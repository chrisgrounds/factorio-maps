...
const UploadPage = () => {
  ...
  const onUpload = async () => {
    const encodedFiles = await Promise
      .all(Array
        .from(files)
  ...
  return (
    <div className="flex flex-col items-center justify-center bg-gray-100">
      <div className="p-8 bg-white shadow-md rounded-lg">
        <h1 className="text-3xl font-bold mb-6 text-center">Upload Your Mapshot</h1>
        ...
        <div>
          {files && Array.from(files).map((file: any) => (
            <div key={file.name} className="flex items-center justify-between w-full p-2 mt-4 bg-gray-100 rounded-lg">
              <p className="text-sm font-semibold text-gray-700">{file.name}</p>
              <p className="text-sm font-semibold text-gray-700">{bytesToMB(file.size)} MB</p>
            </div>
          ))}
        </div>
        <button type="submit" className="w-full p-3 mt-4 bg-indigo-600 text-white rounded shadow hover:bg-indigo-500" onClick={onUpload}>Upload</button>
        {url &&
